...
if ($mediaFile) {
    if ($db && isset($_SESSION['user_id'])) {
        $stmt = $db->prepare('INSERT INTO history (user_id, filename) VALUES (:uid, :filename)');
        $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
        $stmt->bindValue(':filename', $mediaFile, SQLITE3_TEXT);
        $stmt->execute();
    } else {
...
// 收藏和喜欢处理
if ($db && isset($_SESSION['user_id'])) {
    if (isset($_GET['fav']) && isset($_GET['file'])) {
        $file = $_GET['file'];
        if ($_GET['fav'] === '1') {
            $stmt = $db->prepare('INSERT OR IGNORE INTO favorites (user_id, filename) VALUES (:uid, :filename)');
            $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
            $stmt->bindValue(':filename', $file, SQLITE3_TEXT);
            $stmt->execute();
        } elseif ($_GET['fav'] === '0') {
            $stmt = $db->prepare('DELETE FROM favorites WHERE user_id = :uid AND filename = :filename');
            $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
            $stmt->bindValue(':filename', $file, SQLITE3_TEXT);
            $stmt->execute();
        }
    ...
    if (isset($_GET['like']) && isset($_GET['file'])) {
        $file = $_GET['file'];
        if ($_GET['like'] === '1') {
            ...
            $stmt = $db->prepare('INSERT OR IGNORE INTO likes (user_id, filename) VALUES (:uid, :filename)');
            $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
            $stmt->bindValue(':filename', $file, SQLITE3_TEXT);
            $stmt->execute();
        } elseif ($_GET['like'] === '0') {
            $stmt = $db->prepare('DELETE FROM likes WHERE user_id = :uid AND filename = :filename');
            $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
            $stmt->bindValue(':filename', $file, SQLITE3_TEXT);
            $stmt->execute();
        }
...
// 稍后再看处理
if ($db && isset($_SESSION['user_id'])) {
    $db->exec("CREATE TABLE IF NOT EXISTS watch_later (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, filename TEXT, added_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, filename))");
    if (isset($_GET['later']) && isset($_GET['file'])) {
        $file = $_GET['file'];
        if ($_GET['later'] === '1') {
            $stmt = $db->prepare('INSERT OR IGNORE INTO watch_later (user_id, filename) VALUES (:uid, :filename)');
            $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
            $stmt->bindValue(':filename', $file, SQLITE3_TEXT);
            $stmt->execute();
        } elseif ($_GET['later'] === '0') {
            $stmt = $db->prepare('DELETE FROM watch_later WHERE user_id = :uid AND filename = :filename');
            $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
            $stmt->bindValue(':filename', $file, SQLITE3_TEXT);
            $stmt->execute();
        }
...
if ($db && isset($_SESSION['user_id']) && $mediaFile) {
    $fav_res = $db->querySingle("SELECT 1 FROM favorites WHERE user_id = " . intval($_SESSION['user_id']) . " AND filename = '" . SQLite3::escapeString($mediaFile) . "'");
    $is_fav = $fav_res ? true : false;
    $db->exec("CREATE TABLE IF NOT EXISTS likes (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, filename TEXT, liked_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, filename))");
    $like_res = $db->querySingle("SELECT 1 FROM likes WHERE user_id = " . intval($_SESSION['user_id']) . " AND filename = '" . SQLite3::escapeString($mediaFile) . "'");
    $is_like = $like_res ? true : false;
}
// 评论处理
if ($db && isset($_SESSION['user_id']) && $mediaFile) {
    $db->exec("CREATE TABLE IF NOT EXISTS comments (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, filename TEXT, content TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
    if (isset($_POST['comment_content']) && trim($_POST['comment_content'])) {
        $stmt = $db->prepare('INSERT INTO comments (user_id, filename, content) VALUES (:uid, :filename, :content)');
        $stmt->bindValue(':uid', $_SESSION['user_id'], SQLITE3_INTEGER);
        $stmt->bindValue(':filename', $mediaFile, SQLITE3_TEXT);
    ...
    // 获取评论
    $comments = [];
    $stmt = $db->prepare('SELECT c.*, u.nickname, u.username FROM comments c LEFT JOIN users u ON c.user_id = u.id WHERE c.filename = :filename ORDER BY c.created_at DESC');
    $stmt->bindValue(':filename', $mediaFile, SQLITE3_TEXT);
    $res = $stmt->execute();
